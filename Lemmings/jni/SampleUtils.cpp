    ...
    if (program)
    {
        ...
        if (linkStatus != GL_TRUE)
        {
            ...
            if (bufLength)
            {
                ...
                char* glbuf = (char*) malloc(bufLength);         
                if (buf)
                {
                    glGetProgramInfoLog(program, bufLength, NULL, buf);
                    LOG("Could not link program: %s", buf);
                    
                    glGetShaderInfoLog(program, bufLength, NULL, glbuf);
                    LOG("Shader program Log: %s", glbuf);
                    //free(buf);
                    //free(glbuf);
                }
            }
...
void
SampleUtils::setOrthoMatrix(float nLeft, float nRight, float nBottom, float nTop, 
